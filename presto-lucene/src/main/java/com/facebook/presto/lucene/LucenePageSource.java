 ...
 *
 */
public class LucenePageSource implements ConnectorPageSource {

	private static final int ROWS_PER_REQUEST = 4096;
	private final LuceneRecordCursor cursor;
	private final List<Type> types;
	private final PageBuilder pageBuilder;
	private boolean closed;

	public LucenePageSource(RecordSet recordSet) {
		this(requireNonNull(recordSet, "recordSet is null").getColumnTypes(),
				recordSet.cursor());
	}

	public LucenePageSource(List<Type> types, RecordCursor cursor) {
		this.cursor = checkType(cursor, LuceneRecordCursor.class, "cursor");
		this.types = unmodifiableList(
				new ArrayList<>(requireNonNull(types, "types is null")));
				...
				for (int column = 0; column < types.size(); column++) {
					...
					if (cursor.isNull(column)) {
						...
						if (javaType == boolean.class) {
							type.writeBoolean(output,
									cursor.getBoolean(column));
