...
import static com.facebook.presto.lucene.hive.metastore.HiveType.HIVE_LONG;
...
public class HiveColumnHandle implements ColumnHandle {
    ...
    @JsonCreator
    public HiveColumnHandle(@JsonProperty("clientId") String clientId, @JsonProperty("name") String name,
            @JsonProperty("hiveType") HiveType hiveType, @JsonProperty("typeSignature") TypeSignature typeSignature,
            @JsonProperty("hiveColumnIndex") int hiveColumnIndex, @JsonProperty("partitionKey") boolean partitionKey) {
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equals(this.clientId, other.clientId) && Objects.equals(this.name, other.name)
    ...
    public String toString() {
        return toStringHelper(this).add("clientId", clientId).add("name", name).add("hiveType", hiveType)
                .add("hiveColumnIndex", hiveColumnIndex).add("partitionKey", partitionKey).toString();
    }
    ...
    public static HiveColumnHandle updateRowIdHandle(String connectorId) {
        // Hive connector only supports metadata delete. It does not support
        // generic row-by-row deletion.
        // Metadata delete is implemented in Presto by generating a plan for
        // row-by-row delete first,
        // and then optimize it into metadata delete. As a result, Hive
        // connector must provide partial
        // plan-time support for row-by-row delete so that planning doesn't
        // fail. This is why we need
        // rowid handle. Note that in Hive connector, rowid handle is not
        // implemented beyond plan-time.

        // Like partition columns, this column can not be found in data source.
        // Using true for partitionKey,
        // the plan-time support needed just works.
        return new HiveColumnHandle(connectorId, UPDATE_ROW_ID_COLUMN_NAME, HIVE_LONG, BIGINT.getTypeSignature(), -1,
                true);
    }
}
