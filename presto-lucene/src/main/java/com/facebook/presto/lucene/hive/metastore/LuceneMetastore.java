...
public interface LuceneMetastore {
	...
	Set<LucenePrivilegeInfo> getDatabasePrivileges(String user,
			String schemaName);

	Set<LucenePrivilegeInfo> getTablePrivileges(String user, String schemaName,
			String tableName);
	...
	default boolean isDatabaseOwner(String user, String databaseName) {
		...
		if (database.getOwnerType() == USER
				&& user.equals(database.getOwnerName())) {
			return true;
		}
		if (database.getOwnerType() == ROLE
				&& getRoles(user).contains(database.getOwnerName())) {
			return true;
		}
		return false;
	}

	default boolean isTableOwner(String user, String databaseName,
			String tableName) {
