 ...
 */
package com.facebook.presto.storage.hdfs.blockcache;
...
public abstract class CustomBufferedIndexInput extends IndexInput {
    ...
    public void readBytes(byte[] b, int offset, int len, boolean useBuffer) throws IOException {
        ...
        if (buffer[bufferIndex] == null) {
            ...
            if (len <= (bufferSize - bufferPosition)) {
                if (len > 0)
                    System.arraycopy((byte[]) buffer[bufferIndex], bufferPosition, b, offset, len);
                bufferPosition += len;
            } else {
                int available = bufferSize - bufferPosition;
                if (available > 0) {
                    System.arraycopy((byte[]) buffer[bufferIndex], bufferPosition, b, offset, available);
                    offset += available;
                    len -= available;
                ...
                for (int i = 0; i < size; i++) {
                    System.arraycopy((byte[]) buffer[bufferIndex], bufferPosition, b, offset, bufferSize);
                    offset += bufferSize;
                    len -= bufferSize;
                    bufferIndex++;
                }
                if (pos > 0) {
                    System.arraycopy((byte[]) buffer[bufferIndex], bufferPosition, b, offset, pos);
                    offset += pos;
                    len -= pos;
                    bufferPosition += pos;
                }
    ...
    public int readInt() throws IOException {
        ...
        if ((bufferSize - bufferPosition) >= 4) {
            return ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xFF) << 24)
                    | ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xFF) << 16)
                    | ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xFF) << 8)
    ...
    public long readLong() throws IOException {
        ...
        if (8 <= (bufferSize - bufferPosition)) {
            final int i1 = ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff) << 24)
                    | ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff) << 16)
                    | ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff) << 8)
                    | (((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff);
            final int i2 = ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff) << 24)
                    | ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff) << 16)
                    | ((((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff) << 8)
                    | (((byte[]) buffer[bufferIndex])[bufferPosition++] & 0xff);
            ...
            synchronized (buffer) {
                for (int i = 0; i < size; i++) {
                    if (buffer[bufferIndex + i] == null) {
                        ...
                        readInternal((long)((bufferIndex + i) * bufferSize), bytes, 0, bufferSize);
                ...
                if (pos > 0) {
                    if (buffer[bufferIndex + size] == null) {
                        byte[] bytes = store.takeBuffer(bufferSize);
                        readInternal((long)((bufferIndex + size) * bufferSize), bytes, 0, pos);
     ...
     */
    protected abstract void readInternal(long filePointer, byte[] b, int offset, int length) throws IOException;
    ...
    protected int flushBuffer(IndexOutput out, long numBytes) throws IOException {
        ...
        if (toCopy > 0) {
            out.writeBytes((byte[]) buffer[bufferIndex], bufferPosition, toCopy);
