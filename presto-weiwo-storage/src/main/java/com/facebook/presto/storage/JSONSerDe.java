...
import com.facebook.presto.storage.wltea.analyzer.cfg.Configuration;
...
//    private static final Log LOG = LogFactory.getLog(JSONSerDe.class);
//
//    // stores the 1-to-1 mapping of weiwo fields to hive columns
//    public static final String WEIWO_COLS = "weiwodb.schema.mapping";
//
...
//
//    // maps hive columns to fields in a weiwo collection
//    public Map<String, String> hiveToWeiwo;
//
//    // A row represents a row in the Hive table 
//    private List<Object> row = new ArrayList<Object>();
//
//    // JSONWritable to hold documents to be serialized.
...
//        if (tblProps.containsKey(WEIWO_COLS)) {
//            String fieldsStr = tblProps.getProperty(WEIWO_COLS);
//            Map<String, String> rules = parse(fieldsStr);
//
//            // register the hive field mappings to mongo field mappings
//            hiveToWeiwo = new HashMap<String, String>();
...
//
//        // Create the JSONWritable instance for future use.
//        jsonWritable = new JSONWritable();
//    }
//    
//   
//
//    private Map<String, String> parse(String fieldsStr) {
//    	String[] strs = fieldsStr.split("\\s*,\\s*");
...
//
//	/**
//     * Takes in the object represented by JSON for Hive to WEIWO/JSON mapping. Records these mappings and infers upper level mappings from
...
//
//            if (hiveToWeiwo.containsKey(key) && !hiveToWeiwo.get(key).equals(value)) {
//                throw new SerDeException("Ambiguous rule definition for " + key);
//            } else {
//                hiveToWeiwo.put(key.toLowerCase(), value);
//            }
//
//            if (key.contains(".")) {
//                // split by "."
...
//
//                    if (hiveToWeiwo.containsKey(curKey) && !hiveToWeiwo.get(curKey).equals(curValue)) {
//                        throw new SerDeException("Ambiguous rule definition for " + curKey);
//                    } else {
//                        hiveToWeiwo.put(curKey.toLowerCase(), curValue);
//                    }
//
//                    curKey += ".";
//                    curValue += ".";
...
//
//    /**
//     * Given a Writable object of JSON, turn it into a Hive table row
//     */
//    @Override
//    //CHECKSTYLE:OFF
...
//        row.clear();
//
//        // Make sure it's a JSONWritable object
//        if (writable instanceof JSONWritable) {
//            doc = ((JSONWritable) writable).getJson();
//        } else {
//            throw new SerDeException(format("%srequires a JSONWritable object, not%s", getClass(), writable.getClass()));
//        }
//
...
//
//                // get the corresponding field name in weiwo
//                String mapping;
//                if (hiveToWeiwo == null) {
//                	mapping = fieldName;
//                } else {
//                	mapping = hiveToWeiwo.containsKey(fieldName)
//                                   ? hiveToWeiwo.get(fieldName)
//                                   : fieldName;
//                }
//                value = deserializeField(getValue(doc, mapping), fieldTypeInfo, fieldName);
//            } catch (Exception e) {
...
//    }
//
//    private Object getValue(final JSONWritable doc, final String mongoMapping) {
//        if (mongoMapping.contains(".")) {
//            int index = mongoMapping.indexOf('.');
//            JSONWritable object = (JSONWritable) doc.get(mongoMapping.substring(0, index));
...
//                    LOG.warn("JSONWritable does not support unions.");
//                    return null;
//                default:
...
//    	JSONWritable list = (JSONWritable) value;
...
//                } else {
//                    if (hiveToWeiwo.containsKey(hiveMapping)) {
//                        mongoMapping = hiveToWeiwo.get(hiveMapping);
//                    } else {
//                        mongoMapping = ext.length() > 0 && hiveToWeiwo.containsKey(ext)
//                                       ? hiveToWeiwo.get(ext) + "." + fieldName
//                                       : hiveMapping;
//                    }
//                }
//
...
//        jsonWritable.setJson(
//          (JSONObject) serializeStruct(obj, (StructObjectInspector) oi, ""));
//        return jsonWritable;
//    }
//    //CHECKSTYLE:ON
//
...
//
//                if (hiveToWeiwo != null && hiveToWeiwo.containsKey(hiveMapping)) {
//                    String mongoMapping = hiveToWeiwo.get(hiveMapping);
...
//    }
    ...
    public static void getResource() throws IOException{
        //返回读取指定资源的输入流
        InputStream is=JSONSerDe.class.getClass().getResourceAsStream("/config/IKAnalyzer.cfg.xml");
...
//    try {
//        reader = new FileReader(JSONSerDe.class.getClassLoader().getResource("config/IKAnalyzer.cfg.xml").getPath());
...
//    }
        ...
        System.out.println(Configuration.class.getClassLoader().getResource("config/IKAnalyzer.cfg.xml").getPath());

        URI uri = null;
        try {
            uri = new URI("hdfs://10.208.54.138:8020/data/weiwodb");
