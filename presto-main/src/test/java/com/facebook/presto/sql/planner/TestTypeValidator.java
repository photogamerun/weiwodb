...
public class TestTypeValidator {
	private static final TableHandle TEST_TABLE_HANDLE = new TableHandle("test",
	...
	public void setUp() {
		...
		columnE = symbolAllocator.newSymbol("e",
				VarcharType.createVarcharType(3)); // varchar(3), to test type
													// only coercion

		Map<Symbol, ColumnHandle> assignments = ImmutableMap
				.<Symbol, ColumnHandle> builder()
				...
				.put(columnE, new TestingColumnHandle("e")).build();

		baseTableScan = new TableScanNode(newId(), TEST_TABLE_HANDLE,
	...
	public void testValidProject() throws Exception {
		Expression expression1 = new Cast(columnB.toSymbolReference(),
				StandardTypes.BIGINT);
		Expression expression2 = new Cast(columnC.toSymbolReference(),
				StandardTypes.BIGINT);
		Map<Symbol, Expression> assignments = ImmutableMap
				.<Symbol, Expression> builder()
				.put(symbolAllocator.newSymbol(expression1, BIGINT),
						expression1)
				.put(symbolAllocator.newSymbol(expression2, BIGINT),
						expression2)
				.build();
	...
	public void testValidUnion() throws Exception {
		...
		ListMultimap<Symbol, Symbol> mappings = ImmutableListMultimap
				.<Symbol, Symbol> builder().put(outputSymbol, columnD)
				.put(outputSymbol, columnD).build();

		PlanNode node = new UnionNode(newId(),
				ImmutableList.of(baseTableScan, baseTableScan), mappings,
	...
	public void testValidWindow() throws Exception {
		...
		Map<Symbol, Signature> signatures = ImmutableMap.of(windowSymbol,
				new Signature("sum", FunctionKind.WINDOW, ImmutableList.of(),
						ImmutableList.of(), DOUBLE.getTypeSignature(),
						ImmutableList.of(DOUBLE.getTypeSignature()), false));
		Map<Symbol, FunctionCall> functions = ImmutableMap.of(windowSymbol,
				new FunctionCall(QualifiedName.of("sum"),
						ImmutableList.of(columnC.toSymbolReference())));
		WindowNode.Frame frame = new WindowNode.Frame(WindowFrame.Type.RANGE,
		...
		PlanNode node = new WindowNode(newId(), baseTableScan, specification,
				functions, signatures, Optional.empty(), ImmutableSet.of(), 0);
	...
	public void testValidAggregation() throws Exception {
		...
		Map<Symbol, Signature> functions = ImmutableMap.of(aggregationSymbol,
				new Signature("sum", FunctionKind.AGGREGATE, ImmutableList.of(),
						ImmutableList.of(), DOUBLE.getTypeSignature(),
						ImmutableList.of(DOUBLE.getTypeSignature()), false));
		Map<Symbol, FunctionCall> aggregations = ImmutableMap
				.of(aggregationSymbol, new FunctionCall(QualifiedName.of("sum"),
						ImmutableList.of(columnC.toSymbolReference())));

		PlanNode node = new AggregationNode(newId(), baseTableScan,
				ImmutableList.of(columnA, columnB), aggregations, functions,
				ImmutableMap.of(),
				ImmutableList.of(ImmutableList.of(columnA, columnB)), SINGLE,
				Optional.empty(), 0, Optional.empty(), null);
	...
	public void testValidTypeOnlyCoercion() throws Exception {
		Expression expression = new Cast(columnB.toSymbolReference(),
				StandardTypes.BIGINT);
		Map<Symbol, Expression> assignments = ImmutableMap
				.<Symbol, Expression> builder()
				.put(symbolAllocator.newSymbol(expression, BIGINT), expression)
				.put(symbolAllocator.newSymbol(columnE.toSymbolReference(),
						VARCHAR), columnE.toSymbolReference()) // implicit
																// coercion from
																// varchar(3) to
																// varchar
				.build();
	...
	public void testInvalidProject() throws Exception {
		...
		Expression expression2 = new Cast(columnA.toSymbolReference(),
				StandardTypes.INTEGER);
		Map<Symbol, Expression> assignments = ImmutableMap
				.<Symbol, Expression> builder()
				.put(symbolAllocator.newSymbol(expression1, BIGINT),
						expression1) // should be INTEGER
				.put(symbolAllocator.newSymbol(expression1, INTEGER),
						expression2)
				.build();
		PlanNode node = new ProjectNode(newId(), baseTableScan, assignments);
	...
	public void testInvalidAggregationFunctionCall() throws Exception {
		...
		Map<Symbol, FunctionCall> aggregations = ImmutableMap
				.of(aggregationSymbol, new FunctionCall(QualifiedName.of("sum"),
						ImmutableList.of(columnA.toSymbolReference()))); // should
																			// be
																			// columnC
	...
	public void testInvalidAggregationFunctionSignature() throws Exception {
		...
		Map<Symbol, Signature> functions = ImmutableMap.of(aggregationSymbol,
				new Signature("sum", FunctionKind.AGGREGATE, ImmutableList.of(),
						ImmutableList.of(), BIGINT.getTypeSignature(), // should
																		// be
																		// DOUBLE
						ImmutableList.of(DOUBLE.getTypeSignature()), false));
		Map<Symbol, FunctionCall> aggregations = ImmutableMap
	...
	public void testInvalidWindowFunctionCall() throws Exception {
		...
		Map<Symbol, FunctionCall> functions = ImmutableMap.of(windowSymbol,
				new FunctionCall(QualifiedName.of("sum"),
						ImmutableList.of(columnA.toSymbolReference()))); // shoud
																			// be
																			// columnC
		WindowNode.Frame frame = new WindowNode.Frame(WindowFrame.Type.RANGE,
				FrameBound.Type.UNBOUNDED_PRECEDING, Optional.empty(),
	...
	private void assertTypesValid(PlanNode node) {
		TYPE_VALIDATOR.validate(node, TEST_SESSION, createTestMetadataManager(),
				SQL_PARSER, symbolAllocator.getTypes());
	}
