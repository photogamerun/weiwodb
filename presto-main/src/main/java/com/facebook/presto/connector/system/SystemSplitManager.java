...
public class SystemSplitManager implements ConnectorSplitManager {
	...
	public SystemSplitManager(NodeManager nodeManager,
		...
		this.tables = uniqueIndex(tables,
	...
	public ConnectorSplitSource getSplits(
			...
			PushDown predicate) {
		SystemTableLayoutHandle layoutHandle = checkType(layout,
				SystemTableLayoutHandle.class, "layout");
		...
		if (tableDistributionMode == SINGLE_COORDINATOR) {
			...
			ConnectorSplit split = new SystemSplit(tableHandle.getConnectorId(),
					tableHandle, address, constraint);
		...
		for (Node node : nodeSet) {
			splits.add(new SystemSplit(tableHandle.getConnectorId(),
					tableHandle, node.getHostAndPort(), constraint));
		}
