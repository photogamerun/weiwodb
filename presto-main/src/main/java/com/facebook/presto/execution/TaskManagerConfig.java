...
@DefunctConfig({"experimental.big-query-max-task-memory", "task.max-memory",
		"task.http-notification-threads", "task.info-refresh-max-wait"})
public class TaskManagerConfig {
	private boolean verboseStats;
	private boolean taskCpuTimerEnabled = true;
	private DataSize maxPartialAggregationMemoryUsage = new DataSize(16,
			Unit.MEGABYTE);
	...
	private int maxWorkerThreads = Runtime.getRuntime().availableProcessors()
			* 2;
	private Integer minDrivers;
	private Integer initialSplitsPerNode;
	private Duration splitConcurrencyAdjustmentInterval = new Duration(100,
			TimeUnit.MILLISECONDS);

	private DataSize sinkMaxBufferSize = new DataSize(32, Unit.MEGABYTE);
	private DataSize maxPagePartitioningBufferSize = new DataSize(32,
			Unit.MEGABYTE);
	...
	private Duration infoUpdateInterval = new Duration(200,
			TimeUnit.MILLISECONDS);
	...
	public TaskManagerConfig setStatusRefreshMaxWait(
			Duration statusRefreshMaxWait) {
	...
	public TaskManagerConfig setInfoUpdateInterval(
			Duration infoUpdateInterval) {
	...
	public TaskManagerConfig setTaskCpuTimerEnabled(
	...
	public TaskManagerConfig setMaxPartialAggregationMemoryUsage(
			DataSize maxPartialAggregationMemoryUsage) {
	...
	public TaskManagerConfig setOperatorPreAllocatedMemory(
	...
	public TaskManagerConfig setMaxIndexMemoryUsage(
			DataSize maxIndexMemoryUsage) {
	...
	public TaskManagerConfig setSplitConcurrencyAdjustmentInterval(
			Duration splitConcurrencyAdjustmentInterval) {
	...
	public TaskManagerConfig setNewSinkBufferImplementation(
	...
	public TaskManagerConfig setTaskNotificationThreads(
