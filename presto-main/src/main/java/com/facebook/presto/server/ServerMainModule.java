...
import com.facebook.presto.weiwo.data.source.DataSourceResource;
import com.facebook.presto.weiwo.data.source.WeiwoDataSourceCache;
import com.facebook.presto.weiwo.data.source.WeiwoDataSourceManager;
import com.facebook.presto.weiwo.manager.WeiwoZkClientFactory;
import com.facebook.presto.weiwo.manager.index.LuceneIndexManager;
import com.facebook.presto.weiwo.manager.index.LuceneIndexResource;
import com.facebook.presto.weiwo.manager.index.LuceneIndexWriterManager;
import com.facebook.presto.weiwo.manager.index.NodeStateChangeService;
import com.facebook.presto.weiwo.recover.RecoverAdapter;
import com.facebook.presto.weiwo.recover.RecoverManager;
import com.facebook.presto.weiwo.recover.RecoverSource;
import com.facebook.presto.weiwo.recover.WorkerRecoverSource;
...
public class ServerMainModule extends AbstractConfigurationAwareModule {
	...
	public ServerMainModule(SqlParserOptions sqlParserOptions) {
		this.sqlParserOptions = requireNonNull(sqlParserOptions,
				"sqlParserOptions is null");
	}
	...
	protected void setup(Binder binder) {
		...
		if (serverConfig.isCoordinator()) {
			...
			}).toProvider(QueryPerformanceFetcherProvider.class)
					.in(Scopes.SINGLETON);
		} else {
			binder.bind(new TypeLiteral<Optional<QueryPerformanceFetcher>>() {
			}).toInstance(Optional.empty());

			// HACK: this binding is needed by SystemConnectorModule, but will
			// only be used on the coordinator
			binder.bind(QueryManager.class).toInstance(
					newProxy(QueryManager.class, (proxy, method, args) -> {
		...
		binder.bind(InternalNodeManager.class).to(DiscoveryNodeManager.class)
				.in(Scopes.SINGLETON);
		binder.bind(NodeManager.class).to(Key.get(InternalNodeManager.class))
				.in(Scopes.SINGLETON);
		httpClientBinder(binder)
				.bindHttpClient("node-manager", ForNodeManager.class)
				.withTracing().withConfigDefaults(config -> {
					config.setIdleTimeout(new Duration(30, SECONDS));
					config.setRequestTimeout(new Duration(10, SECONDS));
				});
		httpClientBinder(binder)
		        .bindHttpClient("datasource-manager", ForDataSourceManager.class)
		        .withTracing().withConfigDefaults(config -> {
		            config.setIdleTimeout(new Duration(30, SECONDS));
		            config.setRequestTimeout(new Duration(10, SECONDS));
        });

		if (serverConfig.isWeiwomanager()) {
			jaxrsBinder(binder).bind(LuceneIndexResource.class);
			newExporter(binder).export(LuceneIndexResource.class)
					.withGeneratedName();
			binder.bind(NodeStateChangeService.class).in(Scopes.SINGLETON);
			binder.bind(LuceneIndexManager.class).in(Scopes.SINGLETON);
			binder.bind(LuceneIndexWriterManager.class).in(Scopes.SINGLETON);
			binder.bind(WeiwoZkClientFactory.class).in(Scopes.SINGLETON);
			binder.bind(WeiwoTableCache.class).in(Scopes.SINGLETON);
			binder.bind(WeiwoDataSourceCache.class).in(Scopes.SINGLETON);
			binder.bind(WeiwoDataSourceManager.class).in(Scopes.SINGLETON);
			binder.bind(RecoverManager.class).in(Scopes.SINGLETON);
			jaxrsBinder(binder).bind(DataSourceResource.class);
			newExporter(binder).export(DataSourceResource.class)
					.withGeneratedName();
			jaxrsBinder(binder).bind(RecoverSource.class);
			newExporter(binder).export(RecoverSource.class).withGeneratedName();
		}
		// node scheduler
		// TODO: remove from NodePartitioningManager and move to
		// CoordinatorModule
		...
		install(conditionalModule(NodeSchedulerConfig.class,
				config -> LEGACY.equalsIgnoreCase(config.getNetworkTopology()),
				moduleBinder -> moduleBinder.bind(NetworkTopology.class)
						.to(LegacyNetworkTopology.class).in(Scopes.SINGLETON)));
		install(conditionalModule(NodeSchedulerConfig.class,
				config -> FLAT.equalsIgnoreCase(config.getNetworkTopology()),
				moduleBinder -> moduleBinder.bind(NetworkTopology.class)
						.to(FlatNetworkTopology.class).in(Scopes.SINGLETON)));

		// task execution
		jaxrsBinder(binder).bind(TaskResource.class);
		newExporter(binder).export(TaskResource.class).withGeneratedName();
		binder.bind(TaskManager.class).to(SqlTaskManager.class)
				.in(Scopes.SINGLETON);

		// worker recover
		binder.bind(RecoverAdapterInterface.class).to(RecoverAdapter.class)
				.in(Scopes.SINGLETON);
		jaxrsBinder(binder).bind(WorkerRecoverSource.class);
		newExporter(binder).export(WorkerRecoverSource.class)
				.withGeneratedName();
		...
		newExporter(binder).export(ExpressionCompiler.class)
				.withGeneratedName();
		configBinder(binder).bindConfig(TaskManagerConfig.class);
		binder.bind(IndexJoinLookupStats.class).in(Scopes.SINGLETON);
		newExporter(binder).export(IndexJoinLookupStats.class)
				.withGeneratedName();
		binder.bind(AsyncHttpExecutionMBean.class).in(Scopes.SINGLETON);
		newExporter(binder).export(AsyncHttpExecutionMBean.class)
				.withGeneratedName();
		...
		newExporter(binder).export(ExchangeExecutionMBean.class)
				.withGeneratedName();

		// execution
		binder.bind(LocationFactory.class).to(HttpLocationFactory.class)
				.in(Scopes.SINGLETON);
		...
		jsonCodecBinder(binder)
				.bindJsonCodec(MemoryPoolAssignmentsRequest.class);
		...
		binder.bind(PageSourceProvider.class).to(PageSourceManager.class)
				.in(Scopes.SINGLETON);
		newSetBinder(binder, ConnectorPageSourceProvider.class);

		// page sink provider
		binder.bind(PageSinkManager.class).in(Scopes.SINGLETON);
		binder.bind(PageSinkProvider.class).to(PageSinkManager.class)
				.in(Scopes.SINGLETON);
		...
		binder.bind(Metadata.class).to(MetadataManager.class)
				.in(Scopes.SINGLETON);

		// type
		binder.bind(TypeRegistry.class).in(Scopes.SINGLETON);
		binder.bind(TypeManager.class).to(TypeRegistry.class)
				.in(Scopes.SINGLETON);
		...
		jsonBinder(binder).addSerializerBinding(Slice.class)
				.to(SliceSerializer.class);
		jsonBinder(binder).addDeserializerBinding(Slice.class)
				.to(SliceDeserializer.class);
		jsonBinder(binder).addSerializerBinding(Expression.class)
				.to(ExpressionSerializer.class);
		jsonBinder(binder).addDeserializerBinding(Expression.class)
				.to(ExpressionDeserializer.class);
		...
		checkState(prestoVersion != null,
				"presto.version must be provided when it cannot be automatically determined");
		...
		discoveryBinder(binder).bindHttpAnnouncement("presto")
				...
				.addProperty("coordinator",
						String.valueOf(serverConfig.isCoordinator()))
				.addProperty("weiwomanager",
						String.valueOf(serverConfig.isWeiwomanager()))
				.addProperty("datasources",
						nullToEmpty(serverConfig.getDataSources()));
		...
		jsonBinder(binder).addSerializerBinding(Block.class)
				.to(BlockJsonSerde.Serializer.class);
		jsonBinder(binder).addDeserializerBinding(Block.class)
				.to(BlockJsonSerde.Deserializer.class);

		// thread visualizer
		jaxrsBinder(binder).bind(ThreadResource.class);

		// PageSorter
		binder.bind(PageSorter.class).to(PagesIndexPageSorter.class)
				.in(Scopes.SINGLETON);

		// PageIndexer
		binder.bind(PageIndexerFactory.class)
				.to(GroupByHashPageIndexerFactory.class).in(Scopes.SINGLETON);

		// Finalizer
		binder.bind(FinalizerService.class).in(Scopes.SINGLETON);

		// index manager
		jsonCodecBinder(binder).bindJsonCodec(WeiwoDBNodeInfo.class);
		jsonCodecBinder(binder)
				.bindListJsonCodec(listJsonCodec(WeiwoDBNodeInfo.class));
		jsonCodecBinder(binder).bindJsonCodec(WeiwoDBMemoryInfo.class);
		jsonCodecBinder(binder)
				.bindListJsonCodec(listJsonCodec(WeiwoDBMemoryInfo.class));
		jsonCodecBinder(binder).bindJsonCodec(IndexInfo.class);
		jsonCodecBinder(binder)
				.bindListJsonCodec(listJsonCodec(IndexInfo.class));
		jsonCodecBinder(binder)
				.bindJsonCodec(IndexInfoWithNodeInfoSimple.class);
		jsonCodecBinder(binder).bindListJsonCodec(
				listJsonCodec(IndexInfoWithNodeInfoSimple.class));
	}

	@Provides
	@Singleton
	public ServerInfo createServerInfo(NodeVersion nodeVersion,
			NodeInfo nodeInfo, ServerConfig serverConfig) {
	...
	public static ExecutorService createAsyncHttpResponseCoreExecutor() {
		return newCachedThreadPool(
				daemonThreadsNamed("async-http-response-%s"));
	}
	...
	public static BoundedExecutor createAsyncHttpResponseExecutor(
			@ForAsyncHttp ExecutorService coreExecutor,
			TaskManagerConfig config) {
		return new BoundedExecutor(coreExecutor,
				config.getHttpResponseThreads());
	}
	...
	public static ScheduledExecutorService createAsyncHttpTimeoutExecutor(
			TaskManagerConfig config) {
		return newScheduledThreadPool(config.getHttpTimeoutThreads(),
	...
	public static ExecutorService createTransactionFinishingExecutor() {
		return newCachedThreadPool(
				daemonThreadsNamed("transaction-finishing-%s"));
	}
	...
	public static TransactionManager createTransactionManager(
		...
		return TransactionManager.create(config, idleCheckExecutor,
				finishingExecutor);
	}

	private static void bindFailureDetector(Binder binder,
			boolean coordinator) {
		...
		if (coordinator) {
			...
			binder.bind(FailureDetector.class)
					.toInstance(new FailureDetector() {
						@Override
