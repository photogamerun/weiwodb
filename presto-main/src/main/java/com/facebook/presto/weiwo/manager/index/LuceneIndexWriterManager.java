...
public class LuceneIndexWriterManager {
    
    private static final Logger log = Logger.get(LuceneIndexWriterManager.class);
    ...
    @Inject
    public LuceneIndexWriterManager() {
    ...
    public List<IndexInfoWithNodeInfo> getIndexList(String db, String table, List<String> partitions) {
        ...
        readLock();
        try {
            partitions.stream().filter(p -> p != null && p != "").forEach(p -> {
                String key = IndexSplitKeyUtil.getPartitionKey(db, table, p);
