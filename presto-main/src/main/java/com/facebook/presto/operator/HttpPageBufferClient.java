...
public final class HttpPageBufferClient implements Closeable {
	 ...
	 * followed by either requestComplete or clientFinished (if buffer
	 * complete). If the client is closed, requestComplete or bufferFinished may
	 * never be called.
	 * <p/>
	...
	public HttpPageBufferClient(HttpClient httpClient, DataSize maxResponseSize,
			Duration minErrorDuration, URI location,
			ClientCallback clientCallback,
			BlockEncodingSerde blockEncodingSerde,
			ScheduledExecutorService executor) {
		this(httpClient, maxResponseSize, minErrorDuration, location,
				clientCallback, blockEncodingSerde, executor,
	...
	public HttpPageBufferClient(HttpClient httpClient, DataSize maxResponseSize,
			...
			ClientCallback clientCallback,
			BlockEncodingSerde blockEncodingSerde,
			ScheduledExecutorService executor, Stopwatch errorStopwatch) {
		this.httpClient = requireNonNull(httpClient, "httpClient is null");
		this.maxResponseSize = requireNonNull(maxResponseSize,
				"maxResponseSize is null");
		this.minErrorDuration = requireNonNull(minErrorDuration,
				"minErrorDuration is null");
		this.location = requireNonNull(location, "location is null");
		this.clientCallback = requireNonNull(clientCallback,
				"clientCallback is null");
		this.blockEncodingSerde = requireNonNull(blockEncodingSerde,
				"blockEncodingManager is null");
		this.executor = requireNonNull(executor, "executor is null");
		this.errorStopwatch = requireNonNull(errorStopwatch,
				"errorStopwatch is null").reset();
	}
	...
	public synchronized PageBufferClientStatus getStatus() {
		...
		return new PageBufferClientStatus(location, state, lastUpdate,
				rowsReceived.get(), pagesReceived.get(),
				rejectedRows == 0
						? OptionalLong.empty()
						: OptionalLong.of(rejectedRows),
				rejectedPages == 0
						? OptionalInt.empty()
						: OptionalInt.of(rejectedPages),
				requestsScheduled.get(), requestsCompleted.get(),
				requestsFailed.get(), httpRequestState);
	}
	...
	public void close() {
		...
		// abort the output buffer on the remote node; response of delete is
		// ignored
		if (shouldSendDelete) {
			sendDelete();
		}
	...
	private synchronized void sendGetResults() {
		URI uri = HttpUriBuilder.uriBuilderFrom(location)
				.appendPath(String.valueOf(token)).build();
		HttpResponseFuture<PagesResponse> resultFuture = httpClient
				.executeAsync(
						prepareGet()
								.setHeader(PRESTO_MAX_SIZE,
										maxResponseSize.toString())
								.setUri(uri).build(),
		...
		Futures.addCallback(resultFuture, new FutureCallback<PagesResponse>() {
			...
			public void onSuccess(PagesResponse result) {
				...
				// }
				// log.info("size of rows " + totalsize);
				...
				try {
					synchronized (HttpPageBufferClient.this) {
						...
						if (!isNullOrEmpty(taskInstanceId) && !result
								.getTaskInstanceId().equals(taskInstanceId)) {
							// TODO: update error message
							throw new PrestoException(REMOTE_TASK_MISMATCH,
				...
				if (clientCallback.addPages(HttpPageBufferClient.this, pages)) {
					...
					rowsReceived.addAndGet(pages.stream()
							.mapToLong(Page::getPositionCount).sum());
				} else {
					pagesRejected.addAndGet(pages.size());
					rowsRejected.addAndGet(pages.stream()
							.mapToLong(Page::getPositionCount).sum());
				}

				synchronized (HttpPageBufferClient.this) {
					// client is complete, acknowledge it by sending it a delete
					// in the next request
			...
			public void onFailure(Throwable t) {
				...
				if (!(t instanceof PrestoException)
						&& errorDuration.compareTo(minErrorDuration) > 0) {
					String message = format("%s (%s - requests failed for %s)",
							WORKER_NODE_ERROR, uri, errorDuration);
	...
	private synchronized void sendDelete() {
		HttpResponseFuture<StatusResponse> resultFuture = httpClient
				.executeAsync(prepareDelete().setUri(location).build(),
		...
		Futures.addCallback(resultFuture, new FutureCallback<StatusResponse>() {
			...
			public void onFailure(Throwable t) {
				...
				if (!(t instanceof PrestoException)
					...
					t = new PrestoException(REMOTE_BUFFER_CLOSE_FAILED, message,
							t);
				}
				handleFailure(t, resultFuture);
			}
	...
	private void handleFailure(Throwable t,
			HttpResponseFuture<?> expectedFuture) {
	...
	public String toString() {
		...
		return toStringHelper(this).add("location", location).addValue(state)
				.toString();
	}
	...
	public static class PageResponseHandler
			implements
				ResponseHandler<PagesResponse, RuntimeException> {
		private final BlockEncodingSerde blockEncodingSerde;
		...
		@Override
		public PagesResponse handleException(Request request,
				Exception exception) {
		...
		public PagesResponse handle(Request request, Response response) {
			try {
				// no content means no content was created within the wait
				// period, but query is still ok
				// if job is finished, complete is set in the response
				if (response.getStatusCode() == HttpStatus.NO_CONTENT.code()) {
					return createEmptyPagesResponse(getTaskInstanceId(response),
							getToken(response), getNextToken(response),
							getComplete(response));
				}

				// otherwise we must have gotten an OK response, everything else
				// is considered fatal
				if (response.getStatusCode() != HttpStatus.OK.code()) {
					...
					try (BufferedReader reader = new BufferedReader(
						...
						for (int i = 0; i < 1000; i++) {
							...
							if (line == null || body.length()
									+ line.length() > 100 * 1024) {
								break;
							}
							body.append(line + "\n");
						}
					} catch (RuntimeException | IOException e) {
						// Ignored. Just return whatever message we were able to
						// decode
					}
				...
				// invalid content type can happen when an error page is
				// returned, but is unlikely given the above 200
				String contentType = response.getHeader(CONTENT_TYPE);
				if (contentType == null) {
					throw new PageTransportErrorException(
							format("%s header is not set: %s", CONTENT_TYPE,
									response));
				}
				...
				try (SliceInput input = new InputStreamSliceInput(
					...
					List<Page> pages = ImmutableList
							.copyOf(readPages(blockEncodingSerde, input));
				...
				throw new PageTransportErrorException(format(
						"Error fetching %s: %s",
						request.getUri().toASCIIString(), e.getMessage()), e);
			}
		}
		...
		private static long getToken(Response response) {
			...
			if (tokenHeader == null) {
				throw new PageTransportErrorException(
						format("Expected %s header", PRESTO_PAGE_TOKEN));
			}
		...
		private static long getNextToken(Response response) {
			...
			if (nextTokenHeader == null) {
				throw new PageTransportErrorException(
						format("Expected %s header", PRESTO_PAGE_NEXT_TOKEN));
			}
		...
		private static boolean getComplete(Response response) {
			...
			if (bufferComplete == null) {
				throw new PageTransportErrorException(
						format("Expected %s header", PRESTO_BUFFER_COMPLETE));
			}
	...
	public static class PagesResponse {
		public static PagesResponse createPagesResponse(String taskInstanceId,
				long token, long nextToken, Iterable<Page> pages,
				boolean complete) {
			return new PagesResponse(taskInstanceId, token, nextToken, pages,
					complete);
		}

		public static PagesResponse createEmptyPagesResponse(
				String taskInstanceId, long token, long nextToken,
				boolean complete) {
			return new PagesResponse(taskInstanceId, token, nextToken,
					ImmutableList.<Page> of(), complete);
		}
		...
		private PagesResponse(String taskInstanceId, long token, long nextToken,
				Iterable<Page> pages, boolean clientComplete) {
		...
		@Override
		public String toString() {
			return toStringHelper(this).add("token", token)
					.add("nextToken", nextToken).add("pagesSize", pages.size())
					.add("clientComplete", clientComplete).toString();
		}
	}
}
