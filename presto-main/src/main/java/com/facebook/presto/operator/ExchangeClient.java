...
public class ExchangeClient implements Closeable {
	...
	public ExchangeClient(BlockEncodingSerde blockEncodingSerde,
			DataSize maxBufferedBytes, DataSize maxResponseSize,
			int concurrentRequestMultiplier, Duration minErrorDuration,
	...
	public synchronized ExchangeClientStatus getStatus() {
		...
		ImmutableList.Builder<PageBufferClientStatus> exchangeStatus = ImmutableList
				.builder();
		for (HttpPageBufferClient client : allClients.values()) {
			exchangeStatus.add(client.getStatus());
		}
		return new ExchangeClientStatus(bufferBytes, averageBytesPerRequest,
				bufferedPages, noMoreLocations, exchangeStatus.build());
	}
	...
	public Page pollPage() {
		checkState(!Thread.holdsLock(this),
				"Can not get next page while holding a lock on this");
	...
	public Page getNextPage(Duration maxWaitTime) throws InterruptedException {
		...
		if (page == null && maxWaitTime.toMillis() >= 1
				&& !allClients.isEmpty()) {
			page = pageBuffer.poll(maxWaitTime.toMillis(),
	...
	private Page postProcessPage(Page page) {
		...
		if (page == NO_MORE_PAGES) {
			...
			checkState(pageBuffer.add(NO_MORE_PAGES),
					"Could not add no more pages marker");
		...
		if (page != null) {
			synchronized (this) {
				...
				systemMemoryUsageListener.updateSystemMemoryUsage(
	...
	public synchronized void scheduleRequestIfNecessary() {
		...
		for (URI location : locations) {
			if (!allClients.containsKey(location)) {
				HttpPageBufferClient client = new HttpPageBufferClient(
						httpClient, maxResponseSize, minErrorDuration, location,
						new ExchangeClientCallback(), blockEncodingSerde,
						executor);
		...
		int clientCount = (int) ((1.0 * neededBytes / averageBytesPerRequest)
				* concurrentRequestMultiplier);
		clientCount = Math.max(clientCount, 1);

		int pendingClients = allClients.size() - queuedClients.size()
	...
	private synchronized boolean addPages(List<Page> pages) {
		...
		long memorySize = pages.stream().mapToLong(Page::getRetainedSizeInBytes)
				.sum();
		...
		long responseSize = pages.stream().mapToLong(Page::getSizeInBytes)
				.sum();
		// AVG_n = AVG_(n-1) * (n-1)/n + VALUE_n / n
		averageBytesPerRequest = (long) (1.0 * averageBytesPerRequest
	...
	private class ExchangeClientCallback implements ClientCallback {
		@Override
