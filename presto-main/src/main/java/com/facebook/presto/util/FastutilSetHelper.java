...
public final class FastutilSetHelper {
	...
	public static Set<?> toFastutilHashSet(Set<?> set, Type type,
			FunctionRegistry registry) {
		// 0.25 as the load factor is chosen because the argument set is assumed
		// to be small (<10000),
		// and the return set is assumed to be read-heavy.
		// The performance of InCodeGenerator heavily depends on the load factor
		// being small.
		Class<?> javaElementType = type.getJavaType();
		if (javaElementType == long.class) {
			if (type == RealType.REAL) {
				return new DoubleOpenCustomHashSet((Collection<Double>) set,
						0.25f, new DoubleStrategy(registry, type));
			} else {
				return new LongOpenCustomHashSet((Collection<Long>) set, 0.25f,
						new LongStrategy(registry, type));
			}
		}
		if (javaElementType == double.class) {
			return new DoubleOpenCustomHashSet((Collection<Double>) set, 0.25f,
					new DoubleStrategy(registry, type));
		}
		if (javaElementType == boolean.class) {
			...
			return new ObjectOpenCustomHashSet(set, 0.25f,
					new ObjectStrategy(registry, type));
		} else {
			throw new UnsupportedOperationException(
					"Unsupported native type in set: " + type.getJavaType()
							+ " with type " + type.getTypeSignature());
		}
	}
	...
	public static boolean in(Object objectValue,
			ObjectOpenCustomHashSet<?> set) {
	...
	private static final class LongStrategy implements LongHash.Strategy {
		...
		private LongStrategy(FunctionRegistry registry, Type type) {
			hashCodeHandle = registry
					.getScalarFunctionImplementation(registry
							.resolveOperator(HASH_CODE, ImmutableList.of(type)))
					.getMethodHandle();
			equalsHandle = registry
					.getScalarFunctionImplementation(registry.resolveOperator(
							EQUAL, ImmutableList.of(type, type)))
					.getMethodHandle();
		}
	...
	private static final class ObjectStrategy implements Hash.Strategy {
		private final MethodHandle hashCodeHandle;
		private final MethodHandle equalsHandle;
		...
		private ObjectStrategy(FunctionRegistry registry, Type type) {
			...
			equalsHandle = registry
					.getScalarFunctionImplementation(registry.resolveOperator(
							EQUAL, ImmutableList.of(type, type)))
					.getMethodHandle().asType(MethodType.methodType(
		...
		public int hashCode(Object value) {
			try {
				return Ints.checkedCast(Long
						.hashCode((long) hashCodeHandle.invokeExact(value)));
			} catch (Throwable t) {
				Throwables.propagateIfInstanceOf(t, Error.class);
				Throwables.propagateIfInstanceOf(t, PrestoException.class);
				throw new PrestoException(GENERIC_INTERNAL_ERROR, t);
