...
public class CountConstantOptimizer implements PlanOptimizer {
	@Override
	...
	private static class Rewriter extends SimplePlanRewriter<Void> {
		...
		public PlanNode visitAggregation(AggregationNode node,
			...
			Map<Symbol, FunctionCall> aggregations = new LinkedHashMap<>(
					node.getAggregations());
			Map<Symbol, Signature> functions = new LinkedHashMap<>(
					node.getFunctions());

			PlanNode source = context.rewrite(node.getSource());
			if (source instanceof ProjectNode) {
				ProjectNode projectNode = (ProjectNode) source;
				for (Entry<Symbol, FunctionCall> entry : node.getAggregations()
						.entrySet()) {
					...
					if (isCountConstant(projectNode, functionCall, signature)) {
						aggregations.put(symbol,
								new FunctionCall(functionCall.getName(),
										functionCall.isDistinct(),
										ImmutableList.<Expression> of()));
						functions.put(symbol, new Signature("count", AGGREGATE,
		...
		public static boolean isCountConstant(ProjectNode projectNode,
			...
			if (argument instanceof SymbolReference) {
				...
				Expression argumentExpression = projectNode.getAssignments()
						.get(argumentSymbol);
				return (argumentExpression instanceof Literal)
						&& (!(argumentExpression instanceof NullLiteral));
			}
