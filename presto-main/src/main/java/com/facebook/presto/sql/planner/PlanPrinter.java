...
public class PlanPrinter {
	...
	private PlanPrinter(PlanNode plan, Map<Symbol, Type> types,
			Metadata metadata, Session sesion) {
		this(plan, types, metadata, sesion, 0);
	}

	private PlanPrinter(PlanNode plan, Map<Symbol, Type> types,
			Metadata metadata, Session session, int indent) {
	...
	private PlanPrinter(PlanNode plan, Map<Symbol, Type> types,
			...
			Map<PlanNodeId, PlanNodeStats> stats, int indent) {
		requireNonNull(plan, "plan is null");
		requireNonNull(types, "types is null");
		requireNonNull(metadata, "metadata is null");
	...
	public static String textLogicalPlan(PlanNode plan, Map<Symbol, Type> types,
			Metadata metadata, Session session, int indent) {
		return new PlanPrinter(plan, types, metadata, session, indent)
				.toString();
	}
	...
	public static String textLogicalPlan(PlanNode plan, Map<Symbol, Type> types,
			Metadata metadata, Session session,
			Map<PlanNodeId, PlanNodeStats> stats, int indent) {
		return new PlanPrinter(plan, types, metadata, session, stats, indent)
				.toString();
	}

	public static String textDistributedPlan(List<StageInfo> stages,
			Metadata metadata, Session session) {
		...
		for (StageInfo stageInfo : allStages) {
			...
			for (PlanNodeStats stats : planNodeStats) {
				aggregatedStats.merge(stats.getPlanNodeId(), stats,
						PlanNodeStats::merge);
			}

			builder.append(formatFragment(metadata, session,
					stageInfo.getPlan(), Optional.of(stageInfo.getStageStats()),
	...
	private static List<PlanNodeStats> getPlanNodeStats(TaskStats taskStats) {
		...
		for (PipelineStats pipelineStats : taskStats.getPipelines()) {
			...
			List<OperatorStats> operatorSummaries = pipelineStats
			...
			for (int i = 0; i < operatorSummaries.size(); i++) {
				...
				long wall = operatorStats.getAddInputWall().toMillis()
						+ operatorStats.getGetOutputWall().toMillis()
						+ operatorStats.getFinishWall().toMillis();
				wallMillis.merge(planNodeId, wall, Long::sum);

				// An "internal" pipeline like a hash build, links to another
				// pipeline which is the actual output for this plan node
				if (i == operatorSummaries.size() - 1
						&& !pipelineStats.isOutputPipeline()) {
					pipelineOutputBytes.remove(planNodeId);
					pipelineOutputPositions.remove(planNodeId);
				} else {
					// Overwrite whatever we currently have, to get the last
					// operator's stats for this plan node in this pipeline
					pipelineOutputPositions.put(planNodeId,
							operatorStats.getOutputPositions());
					pipelineOutputBytes.put(planNodeId,
							operatorStats.getOutputDataSize().toBytes());
				}
			}

			for (Map.Entry<PlanNodeId, Long> entry : pipelineOutputPositions
					.entrySet()) {
				outputBytes.merge(entry.getKey(),
						pipelineOutputBytes.get(entry.getKey()), Long::sum);
				outputPositions.merge(entry.getKey(), entry.getValue(),
						Long::sum);
			}
		}
		...
		for (Map.Entry<PlanNodeId, Long> entry : wallMillis.entrySet()) {
			if (outputPositions.containsKey(entry.getKey())) {
				stats.add(new PlanNodeStats(entry.getKey(),
						new Duration(entry.getValue(), MILLISECONDS),
						outputPositions.get(entry.getKey()), succinctDataSize(
								outputBytes.get(entry.getKey()), BYTE)));
			} else {
				// It's possible there will be no output stats because all the
				// pipelines that we observed were non-output.
				// For example in a query like SELECT * FROM a JOIN b ON c = d
				// LIMIT 1
				// It's possible to observe stats after the build starts, but
	...
	public static String textDistributedPlan(SubPlan plan, Metadata metadata,
		...
		for (PlanFragment fragment : plan.getAllFragments()) {
			builder.append(formatFragment(metadata, session, fragment,
	...
	private static String formatFragment(Metadata metadata, Session session,
			PlanFragment fragment, Optional<StageStats> stageStats,
		...
		if (stageStats.isPresent()) {
			builder.append(indentString(1))
					.append(format(
							"Cost: CPU %s, Input %d (%s), Output %d (%s)\n",
		...
		PartitioningScheme partitioningScheme = fragment
		...
		List<String> arguments = partitioningScheme.getPartitioning()
				.getArguments().stream().map(argument -> {
					if (argument.isConstant()) {
						...
						String printableValue = castToVarchar(
								...
								metadata, session);
						return constant.getType().getDisplayName() + "("
								+ printableValue + ")";
					}
		...
		if (replicateNulls) {
			builder.append(
					format("Output partitioning: %s (replicate nulls) [%s]\n",
		...
		if (stageStats.isPresent()) {
			builder.append(
					textLogicalPlan(fragment.getRoot(), fragment.getSymbols(),
							metadata, session, planNodeStats.get(), 1))
					.append("\n");
		} else {
			builder.append(textLogicalPlan(fragment.getRoot(),
					fragment.getSymbols(), metadata, session, 1)).append("\n");
		}
	...
	public static String graphvizLogicalPlan(PlanNode plan,
		...
		PlanFragment fragment = new PlanFragment(
				new PlanFragmentId("graphviz_plan"), plan, types,
				SINGLE_DISTRIBUTION, ImmutableList.of(plan.getId()),
				new PartitioningScheme(Partitioning.create(SINGLE_DISTRIBUTION,
						ImmutableList.of()), plan.getOutputSymbols()));
	...
	private void printStats(int indent, PlanNodeId planNodeId) {
		...
		long totalMillis = stats.get().values().stream()
				.mapToLong(node -> node.getWallTime().toMillis()).sum();
		...
		double fraction = (stats.getWallTime().toMillis())
				/ (double) totalMillis;
		...
		String outputString;
		if (stats.getOutputPositions().isPresent()
				&& stats.getOutputDataSize().isPresent()) {
			outputString = format("%s rows (%s)",
					stats.getOutputPositions().get(),
					stats.getOutputDataSize().get());
		} else {
			outputString = "unknown";
		}
		output.append(indentString(indent)).append(
				format("Cost: %s, Output: %s\n", fractionString, outputString));
	}
	...
	private class Visitor extends PlanVisitor<Integer, Void> {
		...
		@Override
		public Void visitExplainAnalyze(ExplainAnalyzeNode node,
				Integer indent) {
			print(indent, "- ExplainAnalyze => [%s]",
		...
		public Void visitJoin(JoinNode node, Integer indent) {
			...
			node.getFilter()
					.ifPresent(expression -> joinExpressions.add(expression));

			print(indent, "- %s[%s] => [%s]", node.getType().getJoinLabel(),
		...
		public Void visitIndexSource(IndexSourceNode node, Integer indent) {
			print(indent, "- IndexSource[%s, lookup = %s] => [%s]",
					node.getIndexHandle(), node.getLookupSymbols(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			for (Map.Entry<Symbol, ColumnHandle> entry : node.getAssignments()
					.entrySet()) {
				if (node.getOutputSymbols().contains(entry.getKey())) {
					print(indent + 2, "%s := %s", entry.getKey(),
							entry.getValue());
				}
			}
			return null;
		...
		public Void visitLimit(LimitNode node, Integer indent) {
			print(indent, "- Limit%s[%s] => [%s]",
					node.isPartial() ? "Partial" : "", node.getCount(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			return processChildren(node, indent + 1);
		}
		...
		public Void visitDistinctLimit(DistinctLimitNode node, Integer indent) {
			print(indent, "- DistinctLimit%s[%s] => [%s]",
					node.isPartial() ? "Partial" : "", node.getLimit(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			return processChildren(node, indent + 1);
		}
		...
		public Void visitAggregation(AggregationNode node, Integer indent) {
			...
			if (node.getSampleWeight().isPresent()) {
				sampleWeight = format("[sampleWeight = %s]",
						node.getSampleWeight().get());
			}

			print(indent, "- Aggregate%s%s%s => [%s]", type, key, sampleWeight,
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			...
			for (Map.Entry<Symbol, FunctionCall> entry : node.getAggregations()
				...
				if (node.getMasks().containsKey(entry.getKey())) {
					print(indent + 2, "%s := %s (mask = %s)", entry.getKey(),
							entry.getValue(),
		...
		public Void visitGroupId(GroupIdNode node, Integer indent) {
			print(indent, "- GroupId%s => [%s]", node.getGroupingSets(),
					formatOutputs(node.getOutputSymbols()));
			return processChildren(node, indent + 1);
		}
		...
		public Void visitMarkDistinct(MarkDistinctNode node, Integer indent) {
			print(indent, "- MarkDistinct[distinct=%s marker=%s] => [%s]",
		...
		public Void visitWindow(WindowNode node, Integer indent) {
			List<String> partitionBy = Lists.transform(node.getPartitionBy(),
					Functions.toStringFunction());

			List<String> orderBy = Lists.transform(node.getOrderBy(),
					input -> input + " " + node.getOrderings().get(input));
			...
			if (!partitionBy.isEmpty()) {
				...
				List<Symbol> notPrePartitioned = node.getPartitionBy()
						.stream().filter(column -> !node
								.getPrePartitionedInputs().contains(column))
						.collect(toImmutableList());
			...
			print(indent, "- Window[%s] => [%s]", Joiner.on(", ").join(args),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());

			for (Map.Entry<Symbol, FunctionCall> entry : node
					.getWindowFunctions().entrySet()) {
				print(indent + 2, "%s := %s(%s)", entry.getKey(),
						entry.getValue().getName(),
						Joiner.on(", ").join(entry.getValue().getArguments()));
			}
			return processChildren(node, indent + 1);
		}
		...
		public Void visitTopNRowNumber(TopNRowNumberNode node, Integer indent) {
			...
			List<String> args = new ArrayList<>();
			args.add(format("partition by (%s)",
					Joiner.on(", ").join(partitionBy)));
			args.add(format("order by (%s)", Joiner.on(", ").join(orderBy)));

			print(indent, "- TopNRowNumber[%s limit %s] => [%s]",
					Joiner.on(", ").join(args),
					node.getMaxRowCountPerPartition(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());

			print(indent + 2, "%s := %s", node.getRowNumberSymbol(),
					"row_number()");
			return processChildren(node, indent + 1);
		}
		...
		public Void visitRowNumber(RowNumberNode node, Integer indent) {
			...
			if (node.getMaxRowCountPerPartition().isPresent()) {
				args.add(format("limit = %s",
						node.getMaxRowCountPerPartition().get()));
			}

			print(indent, "- RowNumber[%s] => [%s]", Joiner.on(", ").join(args),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
		...
		public Void visitTableScan(TableScanNode node, Integer indent) {
			...
			print(indent, "- TableScan[%s, originalConstraint = %s] => [%s]",
					table, node.getOriginalConstraint(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());

			TupleDomain<ColumnHandle> predicate = node.getLayout()
					.map(layoutHandle -> metadata.getLayout(session,
							layoutHandle))
			...
			if (node.getLayout().isPresent()) {
				...
				ConnectorTableLayoutHandle layout = node.getLayout().get()
						.getConnectorHandle();
				if (!table.getConnectorHandle().toString()
						.equals(layout.toString())) {
			...
			if (predicate.isNone()) {
				...
				for (Map.Entry<Symbol, ColumnHandle> assignment : node
				...
				// then, print constraints for columns that are not in the
				// output
				if (!predicate.isAll()) {
					Set<ColumnHandle> outputs = ImmutableSet
							.copyOf(node.getAssignments().values());
		...
		public Void visitFilter(FilterNode node, Integer indent) {
			print(indent, "- Filter[%s] => [%s]", node.getPredicate(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			return processChildren(node, indent + 1);
		}
		...
		@Override
		public Void visitWeiwoPushDown(WeiwoPushDownNode node, Integer indent) {

			print(indent, "- WeiwoPushDown => [%s]",
					node.getWeiwoOutputSymbols());

			print(indent, "- limit [%s] => orderby [%s] ", node.getLimit(),
					node.getOrderby());
			TableHandle table = node.getTable();
			TupleDomain<ColumnHandle> predicate = node.getLayout()
					.map(layoutHandle -> metadata.getLayout(session,
		...
		public Void visitProject(ProjectNode node, Integer indent) {
			print(indent, "- Project => [%s]",
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			for (Map.Entry<Symbol, Expression> entry : node.getAssignments()
					.entrySet()) {
				if (entry.getValue() instanceof SymbolReference
						&& ((SymbolReference) entry.getValue()).getName()
		...
		public Void visitUnnest(UnnestNode node, Integer indent) {
			print(indent, "- Unnest [replicate=%s, unnest=%s] => [%s]",
					formatOutputs(node.getReplicateSymbols()),
					formatOutputs(node.getUnnestSymbols().keySet()),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
		...
		public Void visitOutput(OutputNode node, Integer indent) {
			print(indent, "- Output[%s] => [%s]",
					Joiner.on(", ").join(node.getColumnNames()),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
		...
		public Void visitTopN(TopNNode node, Integer indent) {
			Iterable<String> keys = Iterables.transform(node.getOrderBy(),
					input -> input + " " + node.getOrderings().get(input));

			print(indent, "- TopN[%s by (%s)] => [%s]", node.getCount(),
					Joiner.on(", ").join(keys),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			return processChildren(node, indent + 1);
		...
		public Void visitSort(SortNode node, Integer indent) {
			...
			print(indent, "- Sort[%s] => [%s]", Joiner.on(", ").join(keys),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			return processChildren(node, indent + 1);
		}
		...
		public Void visitTableFinish(TableFinishNode node, Integer indent) {
			print(indent, "- TableCommit[%s] => [%s]", node.getTarget(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
		...
		public Void visitSample(SampleNode node, Integer indent) {
			print(indent, "- Sample[%s: %s] => [%s]", node.getSampleType(),
					node.getSampleRatio(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
			...
			if (node.getScope() == Scope.LOCAL) {
				print(indent, "- LocalExchange[%s%s] (%s) => %s",
						...
						node.getPartitioningScheme().isReplicateNulls()
								...
								: "",
						Joiner.on(", ")
								.join(node.getPartitioningScheme()
										.getPartitioning().getArguments()),
						formatOutputs(node.getOutputSymbols()));
			} else {
				print(indent, "- %sExchange[%s%s] => %s",
						UPPER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL,
								node.getScope().toString()),
						node.getType(),
						node.getPartitioningScheme().isReplicateNulls()
								? " - REPLICATE NULLS"
								: "",
		...
		public Void visitDelete(DeleteNode node, Integer indent) {
			print(indent, "- Delete[%s] => [%s]", node.getTarget(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
		...
		@Override
		public Void visitMetadataDelete(MetadataDeleteNode node,
				Integer indent) {
			print(indent, "- MetadataDelete[%s] => [%s]", node.getTarget(),
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
		...
		@Override
		public Void visitEnforceSingleRow(EnforceSingleRowNode node,
				Integer indent) {
			print(indent, "- Scalar => [%s]",
					formatOutputs(node.getOutputSymbols()));
			printStats(indent + 2, node.getId());
		...
		protected Void visitPlan(PlanNode node, Integer context) {
			throw new UnsupportedOperationException(
					"PlanPrinter not yet implemented: "
		...
		private String formatOutputs(Iterable<Symbol> symbols) {
			return Joiner.on(", ").join(Iterables.transform(symbols,
					input -> input + ":" + types.get(input).getDisplayName()));
		}

		private void printConstraint(int indent, ColumnHandle column,
			...
			if (!constraint.isAll() && domains.containsKey(column)) {
				print(indent, ":: %s",
						formatDomain(simplifyDomain(domains.get(column))));
			}
		}
			...
			domain.getValues().getValuesProcessor().consume(ranges -> {
				for (Range range : ranges.getOrderedRanges()) {
					...
					if (range.isSingleValue()) {
						String value = castToVarchar(type,
								range.getSingleValue(),
								PlanPrinter.this.metadata, session);
						builder.append('[').append(value).append(']');
					} else {
						builder.append((range.getLow()
								.getBound() == Marker.Bound.EXACTLY)
										? '['
										: '(');
						...
						if (range.getLow().isLowerUnbounded()) {
							...
							builder.append(castToVarchar(type,
					...
					.map(value -> castToVarchar(type, value,
							PlanPrinter.this.metadata, session))
					.sorted() // Sort so the values will be printed in
								// predictable order
					.forEach(parts::add), allOrNone -> {
						if (allOrNone.isAll()) {
	...
	private static String castToVarchar(Type type, Object value,
			Metadata metadata, Session session) {
		if (value == null) {
			return "NULL";
		}

		Signature coercion = metadata.getFunctionRegistry().getCoercion(type,
				VARCHAR);

		try {
			Slice coerced = (Slice) new FunctionInvoker(
					metadata.getFunctionRegistry()).invoke(coercion,
	...
	private static class PlanNodeStats {
		...
		public PlanNodeStats(PlanNodeId planNodeId, Duration wallTime,
				long outputPositions, DataSize outputDataSize) {
			this(planNodeId, wallTime, Optional.of(outputPositions),
					Optional.of(outputDataSize));
		}

		private PlanNodeStats(PlanNodeId planNodeId, Duration wallTime,
		...
		public static PlanNodeStats merge(PlanNodeStats planNodeStats1,
				PlanNodeStats planNodeStats2) {
			checkArgument(
					planNodeStats1.getPlanNodeId()
							.equals(planNodeStats2.getPlanNodeId()),
					"planNodeIds do not match. %s != %s",
					planNodeStats1.getPlanNodeId(),
					planNodeStats2.getPlanNodeId());
			Optional<Long> outputPositions;
			if (planNodeStats1.getOutputPositions().isPresent()
					&& planNodeStats2.getOutputPositions().isPresent()) {
				outputPositions = Optional
						.of(planNodeStats1.getOutputPositions().get()
								+ planNodeStats2.getOutputPositions().get());
			...
			Optional<DataSize> outputDataSize;
			if (planNodeStats1.getOutputDataSize().isPresent()
					&& planNodeStats2.getOutputDataSize().isPresent()) {
				outputDataSize = Optional.of(succinctBytes(planNodeStats1
						.getOutputDataSize().get().toBytes()
						+ planNodeStats2.getOutputDataSize().get().toBytes()));
			...
			return new PlanNodeStats(planNodeStats1.getPlanNodeId(),
					new Duration(
							planNodeStats1.getWallTime().toMillis()
									+ planNodeStats2.getWallTime().toMillis(),
							MILLISECONDS),
