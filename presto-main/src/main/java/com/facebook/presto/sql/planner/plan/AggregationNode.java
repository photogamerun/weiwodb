...
public class AggregationNode extends PlanNode {
	...
	private final Symbol distinct2group;
	public enum Step {
		PARTIAL(true, true), FINAL(false, false), INTERMEDIATE(false,
				true), SINGLE(true, false);
	...
	public AggregationNode(@JsonProperty("id") PlanNodeId id,
			...
			@JsonProperty("distinct2group") Symbol distinct2group) {
		super(id);

		this.source = source;
		this.groupByKeys = ImmutableList
				.copyOf(requireNonNull(groupByKeys, "groupByKeys is null"));
		this.aggregations = ImmutableMap
				.copyOf(requireNonNull(aggregations, "aggregations is null"));
		this.functions = ImmutableMap
				.copyOf(requireNonNull(functions, "functions is null"));
		this.masks = ImmutableMap
				.copyOf(requireNonNull(masks, "masks is null"));
		for (Symbol mask : masks.keySet()) {
			checkArgument(aggregations.containsKey(mask),
					"mask does not match any aggregations");
		}

		this.distinct2group = distinct2group;

		this.groupingSets = ImmutableList
				.copyOf(requireNonNull(groupingSets, "groupingSets is null"));
		this.step = step;
		this.sampleWeight = requireNonNull(sampleWeight,
				"sampleWeight is null");
		checkArgument(confidence >= 0 && confidence <= 1,
				"confidence must be in [0, 1]");
		this.confidence = confidence;
		this.hashSymbol = hashSymbol;
	}

	@JsonProperty("distinct2group")
	public Symbol getDistinct2group() {
		return distinct2group;
	}
