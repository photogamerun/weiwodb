...
/**
 * Ensures that all dependencies (i.e., symbols in expressions) for a plan node
 * are provided by its source nodes
 */
public final class ValidateDependenciesChecker
		implements
			PlanSanityChecker.Checker {
	@Override
	public void validate(PlanNode plan, Session session, Metadata metadata,
			SqlParser sqlParser, Map<Symbol, Type> types) {
	...
	private static class Visitor extends PlanVisitor<Void, Void> {
		...
		public Void visitAggregation(AggregationNode node, Void context) {
			...
			checkDependencies(inputs, node.getGroupBy(),
			...
			if (node.getSampleWeight().isPresent()) {
				checkArgument(inputs.contains(node.getSampleWeight().get()),
			...
			for (FunctionCall call : node.getAggregations().values()) {
				Set<Symbol> dependencies = DependencyExtractor
						.extractUnique(call);
				checkDependencies(inputs, dependencies,
		...
		public Void visitGroupId(GroupIdNode node, Void context) {
			...
			checkDependencies(source.getOutputSymbols(), node.getInputSymbols(),
					"Invalid node. Grouping symbols (%s) not in source plan output (%s)",
					node.getInputSymbols(), source.getOutputSymbols());
		...
		public Void visitMarkDistinct(MarkDistinctNode node, Void context) {
			...
			checkDependencies(source.getOutputSymbols(),
		...
		public Void visitWindow(WindowNode node, Void context) {
			...
			checkDependencies(inputs, node.getPartitionBy(),
			...
			checkDependencies(inputs, bounds.build(),
					"Invalid node. Frame bounds (%s) not in source plan output (%s)",
					bounds.build(), node.getSource().getOutputSymbols());
		...
		public Void visitRowNumber(RowNumberNode node, Void context) {
			...
			checkDependencies(source.getOutputSymbols(), node.getPartitionBy(),
					"Invalid node. Partition by symbols (%s) not in source plan output (%s)",
					node.getPartitionBy(), node.getSource().getOutputSymbols());
		...
		public Void visitFilter(FilterNode node, Void context) {
			...
			Set<Symbol> inputs = ImmutableSet.copyOf(source.getOutputSymbols());
			checkDependencies(inputs, node.getOutputSymbols(),
		...
		public Void visitJoin(JoinNode node, Void context) {
			...
			Set<Symbol> leftInputs = ImmutableSet
					.copyOf(node.getLeft().getOutputSymbols());
			Set<Symbol> rightInputs = ImmutableSet
		...
		public Void visitIndexJoin(IndexJoinNode node, Void context) {
			...
			Set<Symbol> probeInputs = ImmutableSet
					.copyOf(node.getProbeSource().getOutputSymbols());
			Set<Symbol> indexSourceInputs = ImmutableSet
					.copyOf(node.getIndexSource().getOutputSymbols());
			for (IndexJoinNode.EquiJoinClause clause : node.getCriteria()) {
				checkArgument(probeInputs.contains(clause.getProbe()),
						"Probe symbol from index join clause (%s) not in probe source (%s)",
						clause.getProbe(),
						node.getProbeSource().getOutputSymbols());
				checkArgument(indexSourceInputs.contains(clause.getIndex()),
						"Index symbol from index join clause (%s) not in index source (%s)",
						clause.getIndex(),
						node.getIndexSource().getOutputSymbols());
			}

			Set<Symbol> lookupSymbols = node.getCriteria().stream()
					.map(IndexJoinNode.EquiJoinClause::getIndex)
					.collect(toImmutableSet());
			Map<Symbol, Symbol> trace = IndexKeyTracer
					.trace(node.getIndexSource(), lookupSymbols);
			checkArgument(
					!trace.isEmpty()
		...
		@Override
		public Void visitWeiwoPushDown(WeiwoPushDownNode node, Void context) {
			verifyUniqueId(node);
			return null;
		}
		...
		public Void visitUnnest(UnnestNode node, Void context) {
			...
			checkDependencies(source.getOutputSymbols(), required,
		...
		public Void visitExchange(ExchangeNode node, Void context) {
			...
			checkDependencies(node.getOutputSymbols(),
			...
			if (node.getSampleWeightSymbol().isPresent()) {
				checkArgument(
						source.getOutputSymbols()
								.contains(node.getSampleWeightSymbol().get()),
						"Invalid node. Sample weight symbol (%s) is not in source plan output (%s)",
		...
		public Void visitDelete(DeleteNode node, Void context) {
			...
			checkArgument(source.getOutputSymbols().contains(node.getRowId()),
					"Invalid node. Row ID symbol (%s) is not in source plan output (%s)",
					node.getRowId(), node.getSource().getOutputSymbols());
		...
		public Void visitApply(ApplyNode node, Void context) {
			...
			checkDependencies(node.getInput().getOutputSymbols(),
					node.getCorrelation(),
					"APPLY input must provide all the necessary correlation symbols for subquery");
			checkDependencies(
					DependencyExtractor.extractUnique(node.getSubquery()),
					node.getCorrelation(),
					"not all APPLY correlation symbols are not used in subquery");
